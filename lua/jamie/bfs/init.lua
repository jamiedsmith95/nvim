...
local config = require "jamie.bfs.config"
local keymaps = require("jamie.bfs.keymaps")
...
M.set_buffers = function(buf)
  for i, b in ipairs(get_bufs()) do
    ...
    local max_width = config.jamie_config.width
    local linenr = b.lnum
    local empty = {}
...
M.open = function()
  ...
  if not M.main_buf and not M.main_win then
    ...
    M.main_win = vim.api.nvim_open_win(M.main_buf, 1, config.jamie_config)
